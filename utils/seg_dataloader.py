...
class SegDataLoader(object):
    def __init__(self, main_dir, batch_size, resize_shape, crop_shape, paths_file, buffer_size=100, split='train'):
        ...
        if split == 'train':
            ...
            data_tr = data_tr.map(self.parse_train, num_parallel_calls=16)
        else:
            # , num_threads=8, output_buffer_size=100*self.batch_size)
            data_tr = data_tr.map(self.parse_val, num_parallel_calls=16)
    ...
    def parse_train(self, im_path, label_path):
        ...
        img = tf.image.resize_images(
            img, self.resize_shape, method=tf.image.ResizeMethod.BICUBIC)
...
if __name__ == "__main__":
    ...
    with tf.device('/cpu:0'):
        ...
        segdl = SegDataLoader('/home/eren/Data/Cityscapes/',
                              10, (512, 1024), (512, 512), 'val.txt', split='val')
